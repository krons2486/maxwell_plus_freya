...
pub fn AddObjectDialogApp() -> Element {
    ...
    let handle_add = {
        ...
        move |_: freya::events::MouseEvent| {
            ...
            if x1_str.chars().all(|c| c.is_whitespace()) || y1_str.chars().all(|c| c.is_whitespace()) {
            ...
            if object_type == ObjectType::Rectangle {
                ...
                if x2_str.chars().all(|c| c.is_whitespace()) || y2_str.chars().all(|c| c.is_whitespace()) {
...
pub fn RectangleDialogApp() -> Element {
    ...
    let handle_create = {
        ...
        move |_: freya::events::MouseEvent| {
            ...
            if x1_str.is_empty() || y1_str.is_empty() || x2_str.is_empty() || y2_str.is_empty() || eps_str.is_empty() || mu_str.is_empty() {
            ...
            // Проверяем разумность значений
            if x1_val.is_nan() || y1_val.is_nan() || x2_val.is_nan() || y2_val.is_nan() || eps_val.is_nan() || mu_val.is_nan() {
...
pub fn launch_dialog_app() {
    launch_with_props(AddObjectDialogApp, "Настройки проекта", (600.0, 675.0));
}

/// Запуск диалогового окна для создания прямоугольника
pub fn launch_rectangle_dialog() {
    launch_with_props(RectangleDialogApp, "Создание прямоугольника", (400.0, 300.0));
}

/// Запуск диалогового окна для создания источника
pub fn launch_source_dialog() {
    launch_with_props(SourceDialogApp, "Создание источника", (300.0, 200.0));
}

/// Запуск диалогового окна для создания зонда
pub fn launch_probe_dialog() {
    launch_with_props(ProbeDialogApp, "Создание зонда", (300.0, 200.0));
}